...
2. git cloneコマンドでGitリポジトリをクローンしてください。
```
...
mailhogを使用してメールを送信できるようにするために.envファイルの以下の項目を記述してください。
```
    MAIL_MAILER=smtp
    MAIL_HOST=mail
    MAIL_PORT=1025
...
## 管理者ユーザーのアカウント
以下のユーザー情報は管理者ユーザーのアカウント情報です。
管理者用ログイン画面にてログインする際は以下のユーザー名のアカウントでログインしてください。
ちなみに以下のアカウントでは一般ユーザー画面でもログインすることができます。
...
1. 管理者ログイン画面でログインする管理者ユーザーはセキュリティ面を考慮し,ミドルウェアを使用して前述の「松永 菜月」の管理者ユーザーでのみログイン可能となっております。
その他のmailhogで作成した新規ユーザーは全て一般ユーザーのアカウントとなっており、管理者用ログイン画面で使用することができません。
管理者ログイン画面で使用した場合、ミドルウェアでのバリデーションによって「管理者のみログイン可能です」という文言が表示される仕様となります。
